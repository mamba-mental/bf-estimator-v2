...
def run_user_interaction(use_test_data=False, user_data=None):
    # If test data is provided, use it instead of gathering user input.
    if use_test_data:
        if user_data:
            logger.debug("Using provided user_data for test mode")
            return process_test_data(user_data)
        else:
            logger.debug("Using default TEST_DATA for test mode")
            return process_test_data(TEST_DATA)

    # If user_data is provided directly, process it
    if user_data:
        logger.debug("Using provided user_data for processing")
        return process_test_data(user_data)
