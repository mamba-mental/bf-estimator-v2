...
def run_user_interaction(use_test_data=False, user_data=None):
    logger.info("Starting run_user_interaction")
    logger.info(f"use_test_data: {use_test_data}, user_data provided: {user_data is not None}")
    
    if user_data is not None:
        # Process user data directly if provided
        logger.info("Processing provided user data")
        ...
        volume_score, intensity_score, frequency_score = calculate_lean_mass_preservation_scores(
            workout_days, workout_type_str)

        # Create user data dictionary for processing
        user_data = {
            'name': name,
        ...
        logger.info("Processing gathered interactive data")
        logger.debug(f"Interactive user data: {user_data}")
        return process_test_data(user_data)
...
def print_summary(progression, initial_data):
    ...
    """
    try:
        # Generate report with resource cleanup
        report_data = generate_comprehensive_report(progression, initial_data)
        
        # Save only one format to reduce file generation
        # Prefer PDF but fallback to MD if PDF fails
        saved_files = {}
        try:
            saved_files = save_report(report_data, initial_data['name'], 'pdf')
            if saved_files and 'pdf' in saved_files:
                print(f"\nPDF report saved to: {saved_files['pdf']}")
            else:
                raise Exception("PDF generation returned no files")
        except Exception as e:
            logger.error(f"Error saving PDF report: {e}")
            print("PDF generation failed - falling back to Markdown format")
...
def main():
    ...
    try:
        ...
        # Ensure matplotlib figures are cleaned up even on error
        plt.close('all')
        raise
